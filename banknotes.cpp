...
signed main () {
    ...
    while (t--) {
        ...
        vi vec (n);
        rep (i,0,n) cin >> vec[i];
        vi number_of_notes (n);
        rep (i,0,n - 1) number_of_notes[i] = nine(vec[i + 1] - vec[i]);
        number_of_notes[n - 1] = 1e18;
        // rep (i,0,n) cout << number_of_notes[i] << " ";
        // cout << "\n";
        int burles {};
        rep (i,0,n) {
            int temp = min(k + 1, number_of_notes[i]);
            k -= temp;
            burles += temp * bexp(10, vec[i]);
            if (k < 0) break;
        }
        cout << burles << "\n";
    }
