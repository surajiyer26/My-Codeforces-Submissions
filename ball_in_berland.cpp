...
signed main () {
	...
	while (t--) {
		...
		vii pairs (k);
		rep (i,0,k) pairs[i].first = boys[i], pairs[i].second = girls[i];
		map <int, int> mboys, mgirls;
		rep (i,0,k) mboys[boys[i]] += 1;
		rep (i,0,k) mgirls[girls[i]] += 1;
		map <pair<int, int>, int> mpairs;
		rep (i,0,k) mpairs[pairs[k]] += 1;
		int ans {};
		rep (i,0,k) {
			mboys[boys[i]] = max(int(0),mboys[boys[i]] - 1);
			mgirls[girls[i]] = max(int(0),mgirls[girls[i]] - 1);
			mpairs[pairs[i]] = max(int(0),mpairs[pairs[i]] - 1);
			ans += k - i - 1 - mboys[boys[i]] - mgirls[girls[i]] + mpairs[pairs[i]];
		}
		cout << ans << "\n";
	}
