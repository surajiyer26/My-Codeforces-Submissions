...
signed main () {
	...
	while (t--) {
		...
		int w, h;
		cin >> w >> h;
		if (x2 - x1 + w > n && y2 - y1 + h > m) cout << -1 << "\n";
		else if (x2 - x1 + w > n) cout << min(max((int)0,h-y1),max((int)0,h-m+y2)) << "\n";
		else if (y2 - y1 + h > m) cout << min(max((int)0,w-x1),max((int)0,w-n+x2)) << "\n";
		else cout << (min(min(max((int)0,h-y1),max((int)0,w-x1)), min(max((int)0,h-m+y2),max((int)0,w-n+x2)))) << "\n";
	}
