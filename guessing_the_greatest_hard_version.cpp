...
signed main () {
    ...
    int n;
    cin >> n;
    int l = 1, r = n, second_largest = ask(l, r);
    if (second_largest > 1 && ask(l, second_largest) == second_largest) {
        r = second_largest;
        while (r - l != 1) {
            int mid = l + (r - l) / 2;
            if (ask(mid, second_largest) == second_largest) l = mid;
            else r = mid;
        }
        cout << "! " << l << endl;
    }
        ...
        while (r - l != 1) {
            int mid = l + (r - l) / 2;
            if (ask(second_largest, mid) == second_largest) r = mid;
            else l = mid;
        }
        cout << "! " << r << endl;
    }
