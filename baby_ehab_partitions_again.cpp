...
signed main () {
    ...
    if (sum % 2 == 0) {
        int dp [n][sum + 1];
        rep (i,0,n) rep (j,0,sum + 1) dp[i][j] = 0;
        dp[0][0] = 1;
        dp[0][vec[0]] = 1;
        rep (i,1,n) {
            rep (j,0,sum + 1) {
                dp[i][j] = dp[i - 1][j];
                if (j >= vec[i]) dp[i][j] = max(dp[i][j], dp[i - 1][j - vec[i]]);
            }
        }
        is_solved = (dp[n - 1][sum / 2] == 0);
    }
    if (is_solved || sum % 2) {
        cout << 0;
        return 0;
    ...
    vi lsb_position (n);
    rep (i,0,n) lsb_position[i] = calc(vec[i]);
    cout << 1 << "\n" << min_element(all(lsb_position)) - lsb_position.begin() + 1;
