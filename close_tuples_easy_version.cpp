...
signed main () {
	...
	while (t--) {
		...
		rep (i,0,n) cin >> vec[i];
		vi vecc (n + 1);
		rep (i,0,n) vecc[vec[i]] += 1;

		int res = 0;

		for (int i = 2; i < n; i++) {
	    	res += vecc[i - 1] * vecc[i] * vecc[i + 1];
	  	}
	  	for (int i = 1; i < n; i++) {
	    	res += vecc[i] * (vecc[i] - 1) / 2 * vecc[i + 1];
	  	}	
	  	for (int i = 2; i <= n; i++) {
	    	res += vecc[i - 1] * vecc[i] * (vecc[i] - 1) / 2;
	  	}
	  	for (int i = 2; i < n; i++) {
	    	res += vecc[i - 1] * vecc[i + 1] * (vecc[i + 1] - 1) / 2;
	  	}
	  	for (int i = 2; i < n; i++) {
	    	res += vecc[i + 1] * vecc[i - 1] * (vecc[i - 1] - 1) / 2;
	  	}
	  	for (int i = 1; i <= n; i++) {
	    	res += vecc[i] * (vecc[i] - 1) * (vecc[i] - 2) / 6;
	  	}
