...
signed main () {
	...
	while (t--) {
		...
		vi vec (n);
		rep (i,0,n) cin >> vec[i];
		int mnx = *min_element(all(vec));
		bool dont_solve_further = false;
		rep (i,0,n) if ((vec[i] & mnx) != mnx) {
			cout << 0 << "\n";
			dont_solve_further = true;
			break;
		}
		if (dont_solve_further) continue;
		int cnt = count(all(vec), mnx);
		cout << (((cnt * (cnt - 1)) % M) * fact[n - 2]) % M << "\n";
	}
