...
signed main () {
	...
	while (t--) {
		...
		rep (i,0,n) cin >> vec[i];
		int odd {}, even {};
		rep (i,0,n) if (i % 2) odd += vec[i]; else even += vec[i];
		if (odd <= accumulate(all(vec), int(0)) / 2) {
			rep (i,0,n) if (i % 2) cout << 1 << " "; else cout << vec[i] << " ";
		}
		else {
			rep (i,0,n) if (i % 2) cout << vec[i] << " "; else cout << 1 << " ";
		}
		cout << "\n";
	}
