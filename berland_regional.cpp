...
signed main () {
	...
	while (t--) {
		...
		vvi vec (n);
		rep (i,0,n) vec[u[i] - 1].pb(s[i]);
		rep (i,0,n) sort(allr(vec[i]));
		rep (i,0,n) rep (j,1,vec[i].size()) vec[i][j] += vec[i][j - 1];

		vi res (n);
		rep (i,0,n) rep (k,1,vec[i].size() + 1) res[k - 1] += vec[i][(vec[i].size() / k) * k - 1];
