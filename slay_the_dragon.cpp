...
signed main () {
	...
	rep (i,0,n) cin >> vec[i];
	sort(all(vec));
	int d = accumulate(all(vec), (int)0);
	int m;
	cin >> m;
	while (m--) {
		int x, y;
		cin >> x >> y;
		int a = lower_bound(all(vec), x) - vec.begin();
		int ans = 2e18;
		if (a > 0) ans = min(ans, (x - vec[a - 1]) + max((int)0, y - d + vec[a - 1]));
    	if (a < n) ans = min(ans, max((int)0, y - d + vec[a]));
    	cout << ans << "\n";
	}
