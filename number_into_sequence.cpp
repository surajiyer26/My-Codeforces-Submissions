...
signed main () {
    ...
    while (t--) {
        ...
        map <int, int> m;
        rep (i,0,prime_factors.size()) m[prime_factors[i]] += 1;
        int mxoccur = 0, cnt = 0;
        for (auto mm: m) {
            if (mm.second > cnt) mxoccur = mm.first, cnt = mm.second;
        }
        vi answer;
        while (nn % mxoccur == 0) {
            answer.pb(mxoccur);
        ...
        if (answer.size()) answer[answer.size() - 1] *= nn;
        else answer.pb(nn);
        cout << answer.size() << "\n";
        rep (i,0,answer.size()) cout << answer[i] << " ";
        cout << "\n";
    }
