...
int check (int i) {
	if ((i > 0) && (i < n - 1) && ((vec[i] > vec[i - 1] && vec[i] > vec[i + 1]) || (vec[i] < vec[i - 1] && vec[i] < vec[i + 1]))) return 1;
	return 0;
}
...
signed main () {
	...
	while (t--) {
		...
		int s = 0;
		rep (i,1,n - 1) {
			if (check(i)) resvec[i] = 1, s += 1;
		}
		int ans = s;
		rep (i,1,n - 1) {
			int temp = vec[i];
			vec[i] = vec[i - 1];
			ans = min(ans, s - resvec[i - 1] - resvec[i] - resvec[i + 1] + check(i - 1) + check(i) + check(i + 1));
			vec[i] = vec[i + 1];
			ans = min(ans, s - resvec[i - 1] - resvec[i] - resvec[i + 1] + check(i - 1) + check(i) + check(i + 1));
			vec[i] = temp;
		}
