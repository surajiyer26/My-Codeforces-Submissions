...
bool solve () {
    ...
    rep (i,0,s.size()) vec[s[i] - '0'].pb(i);
    int start {}, temp {};
    rep (i,0,m) {
        for (char c {l[i]}; c <= r[i]; c++) {
            if (vec[c - '0'].size() == 0) return true;
            if (lower_bound(all(vec[c - '0']), start) - vec[c - '0'].begin() == vec[c - '0'].size()) return true;
            int x = *lower_bound(all(vec[c - '0']), start);
            temp = max(x, temp);
        }
        start = temp + 1;
    }
