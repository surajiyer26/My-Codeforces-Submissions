...
signed main () {
	...
	while (t--) {
		...
		int k;
		vi temp (n);
		rep (i,0,n) temp[i] = vec1[i] - vec2[i];
		int l;
		rep (i,0,n) if (temp[i]!=0) {
			l = i;
			break;
		}
		int r;
		repr (i,n-1,-1) if (temp[i]!=0) {
			r = i;
			break;
		}
		repr (i,l,-1) {
			if (i == 0) break;
			if (vec2[i-1]<=vec2[i] && vec2[i-1]==vec1[i-1]) l = i-1;
			else break;
		}
		rep (i,r,n) {
			if (i == n-1) break;
			if (vec2[i+1]>=vec2[i] && vec2[i+1]==vec1[i+1]) r = i+1;
			else break;
		}
		cout << l+1 << " " << r+1 << "\n";
	}
