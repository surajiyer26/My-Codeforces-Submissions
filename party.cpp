...
int find_label (int u) {
    if (u == label[u]) return u;
    return label[u] = find_label(label[u]);
}
...
int bfs (int n) {
    ...
    while (q.size()) {
        ...
        rep (i,0,k) {
            ...
            q.pop();
            vis[x] = 1;
            for (auto child: adj[x]) if (!vis[child]) q.push(child);
        }
        cnt += 1;
    }
    rep (i,0,vis.size()) vis[i] = 0;
    return cnt;
}
...
signed main () {
    ...
    rep (i,1,n + 1) {
        if (vec[i] != -1) merge(i, vec[i]);
    }
    rep (i,1,n + 1) {
        if (vec[i] == -1) {
            int x = find_label(i);
            rep (j,0,2006) {
                if (label[j] == x) label[j] = i;
            }
        }
    }
    set <int> s;
    rep (i,1,n + 1) s.insert(find_label(i));
    int cnt {0};
    for (auto ss: s) cnt = max(cnt, bfs(ss));
    cout << cnt;
