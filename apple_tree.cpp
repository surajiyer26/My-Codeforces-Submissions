...
int dfs (int node, vvi& adj, vi& visited, vi& leaf) {
	visited[node] = 1;
	bool is_leaf = true;
	for (auto neighbour: adj[node]) if (!visited[neighbour]) {
		is_leaf = false;
		leaf[node] += dfs(neighbour, adj, visited, leaf);
	}
	if (is_leaf) leaf[node] = 1;
	return leaf[node];
} 
...
signed main () {
	...
	while (t--) {
		...
		vi visited (n, 0);
		vi leaf (n, 0);
		rep (i,0,n) if (!visited[i]) leaf[i] = dfs(i, adj, visited, leaf);
		int q;
		cin >> q;
		rep (i,0,q) {
			int u, v;
