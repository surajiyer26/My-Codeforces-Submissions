...
signed main () {
    ...
    while (t--) {
        ...
        int smallest = 2e18, greatest = -2e18;
        rep (i,0,n) smallest = min(smallest, vec[i]), greatest = max(greatest, vec[i]);

        if (abs(smallest) > abs(greatest)) {
            int idx = -1;
            rep (i,0,n) if (vec[i] == smallest) {
                idx = i;
                break;
            }
