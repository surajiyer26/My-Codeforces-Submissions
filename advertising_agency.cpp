...
int ncrdirect (int n, int r) {
    int res = 1;
    for (int i = 1; i <= min(r, n - r); i++) res = (((res * (n - i + 1)) % M) * inv(i, M)) % M;
    return res;
}
...
signed main () {
    ...
    while (t--) {
        ...
        map <int, int> m2;
        rep (i,0,k) m[vec[i]] += 1;
        rep (j,k,n) m2[vec[j]] += 1;
        if (m[vec[k-1]] && m2[vec[k-1]]) {
            int spots = m[vec[k-1]];
            int candidates = m[vec[k-1]] + m2[vec[k-1]];
            cout << ncrdirect(candidates, spots) << "\n";
        }
        else cout << 1 << "\n";
    }
