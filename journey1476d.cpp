...
signed main () {
	...
	while (t--) {
		...
		string s;
		cin >> n >> s;
		vi to_the_right (n, 0), to_the_left (n, 0);
		rep (i,1,n) if (s[i] != s[i - 1]) to_the_right[i] = 1 + to_the_right[i - 1];
		repr (i,n - 2, -1) if (s[i] != s[i + 1]) to_the_left[i] = 1 + to_the_left[i + 1];
		vi vec (n + 1, 0);
		vec[0] = 1 + (s[0] == 'R' ? 1 + to_the_left[0] : 0);
		vec[n] = 1 + (s[n - 1] == 'L' ? 1 + to_the_right[n - 1] : 0);
		rep (i,1,n) vec[i] = 1 + (s[i] == 'R' ? 1 + to_the_left[i] : 0) + (s[i - 1] == 'L' ? 1 + to_the_right[i - 1] : 0);
		rep (i,0,n + 1) cout << vec[i] << " ";
		cout << "\n";
	}
