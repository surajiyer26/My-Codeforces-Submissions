...
signed main () {
    ...
    while (t--) {
        ...
        if (s[0] == '1') dp[0][0] = 0, dp[0][1] = 1;
        if (s[0] == '?') dp[0][0] = 1, dp[0][1] = 1;
        int res = max(dp[0][0], dp[0][1]);

        rep (i,1,n) {
            if (s[i] == '1') dp[i][0] = 0, dp[i][1] = 1 + dp[i - 1][0];
            else if (s[i] == '0') dp[i][0] = 1 + dp[i - 1][1], dp[i][1] = 0;
            else dp[i][0] = 1 + dp[i - 1][1], dp[i][1] = 1 + dp[i - 1][0];
