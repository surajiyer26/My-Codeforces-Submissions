...
bool solve () {
    ...
    cin >> n;
    vi vec (n * 2);
    rep (i,0,n * 2) cin >> vec[i];
    map<int, int> m;
    rep (i,0,n * 2) m[vec[i]] += 1;
    if (m.size() != n) return false;
    for (auto mm: m) if (mm.second != 2) return false;
    vec.clear();
    for (auto mm: m) vec.pb(mm.first);
    sort(allr(vec));
    int cnt = 0;
    vi vecc;
    rep (i,0,n) {
        vec[i] -= 2 * cnt;
        if (vec[i] % (2 * (n - i)) != 0) return false;
        int x = vec[i] / (2 * (n - i));
        cnt += x;
        vecc.pb(x);
    }
    if (vecc[n - 1] <= 0) return false;
    return true;
}
