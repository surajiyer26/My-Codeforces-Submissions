...
signed main () {
	...
	while (t--) {
		...
		vi vec (n);
		rep (i,0,n) cin >> vec[i];
		int one = find(all(vec), int(1)) - vec.begin();
		int two = find(all(vec), int(2)) - vec.begin();
		int mxni = find(all(vec), int(n)) - vec.begin();
		if (one < mxni && two < mxni) {
			if (one < two) cout << two + 1 << " " << mxni + 1 << "\n";
			else cout << one + 1 << " " << mxni + 1 << "\n";
		}
		else if (one > mxni && two > mxni) {
			if (one > two) cout << two + 1 << " " << mxni + 1 << "\n";
			else cout << one + 1 << " " << mxni + 1 << "\n";
		}
		else cout << 1 << " " << 1 << "\n";
