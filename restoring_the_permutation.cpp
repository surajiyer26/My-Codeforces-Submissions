...
signed main () {
	...
	while (t--) {
		...
		multiset<int> minms;
		multiset<int, greater<int>> maxms;
		int idx = 0;
		rep (i,0,n) {
			if (i == 0 || vec[i] != vec[i - 1]) {
				...
				minvec[i] = *minms.begin();
				maxvec[i] = *maxms.begin();
				minms.erase(minms.begin());
				maxms.erase(maxms.begin());
			}
		}
		rep (i,0,n) cout << minvec[i] << " ";
		cout << "\n";
		rep (i,0,n) cout << maxvec[i] << " ";
		cout << "\n";
	}
