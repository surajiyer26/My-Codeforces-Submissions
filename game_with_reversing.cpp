...
signed main () {
	...
	while (t--) {
		...
		string s1, s2;
		cin >> s1 >> s2;
		int change_without_reverse {};
		rep (i,0,n) if (s1[i] != s2[i]) change_without_reverse += 1;
		reverse(all(s2));
		int change_with_reverse {};
		rep (i,0,n) if (s1[i] != s2[i]) change_with_reverse += 1;
		change_without_reverse = 2 * change_without_reverse - (change_without_reverse % 2);
		if (change_with_reverse) change_with_reverse = 2 * change_with_reverse - (1 - change_with_reverse % 2);
		else change_with_reverse = 2;
		cout << min(change_without_reverse, change_with_reverse) << "\n";
	}
