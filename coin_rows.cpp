...
signed main () {
	...
	while (t--) {
		...
		rep (i,0,2) rep (j,1,n) cin >> arr[i][j];
		rep (j,1,n) arr[0][j]+=arr[0][j-1], arr[1][j]+=arr[1][j-1];
		int mn = INT_MAX;
		rep (mxi,0,n) {
			if (max(arr[0][n-1]-arr[0][mxi],arr[1][mxi-1])<mn) {
				mn = max(arr[0][n-1]-arr[0][mxi],arr[1][mxi-1]);
			}
		}
		cout << mn << "\n";
	}
