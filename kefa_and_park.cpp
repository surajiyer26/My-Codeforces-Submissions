...
void dfs (int node, int till_now, int par) {
	if (vec[node]) till_now += 1;
	else till_now = 0;
	if (till_now > m) return;

	if (adj[node].size() == 1 && node != 1) cnt += 1;
	else for (auto child: adj[node]) if (child != par) dfs(child, till_now, node);
}
