...
vi adj [150005];
int label [150005]; // current component of each vertex
int siz [150005]; // size of each component (labels)
int nedges [150005]; // number of edges in each component

void relabel (int v, int target) { // relabel vertex v and neighbours to target
	if (label[v] == target) return;
	label[v] = target;
	for (auto x: adj[v]) relabel (x, target);
}
...
signed main () {
	...
	bool val = true;
	rep (i,0,n) if (nedges[label[i]] != siz[label[i]] * (siz[label[i]] - 1) / 2) val = false;
	if (val) cout << "YES"; else cout << "NO";
