...
signed main () {
	...
	while (t--) {
		...
		map<int, int> m2;
		for (auto mm: m) m2[mm.second] += 1;
		// for (auto mm: m2) cout << mm.first << " " << mm.second << "\n";

		vec.clear();
		for (auto mm: m2) vec.pb(mm.first * mm.second);
		n = vec.size();
		...
		vi num_of_groups;
		for (auto mm: m2) num_of_groups.pb(mm.second);
		rep (i,1,n) num_of_groups[i] += num_of_groups[i - 1];

		for (auto mm: m2) {
			// chosen freq -> m2.first
			int temp = 0;
			if (curr > 0) temp += vec[curr - 1];
			temp += (vec[n - 1] - vec[curr]) - mm.first  * (num_of_groups[n -1] - num_of_groups[curr]);
			res = min(res, temp);
			curr += 1;
		}
		cout << res << "\n";
