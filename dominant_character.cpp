...
int rabin_karp (string s, string t) { int n = s.length(), m = t.length(); long long p = 1; for (int i = 0; i < m - 1; i++) p = (p * 31) % M; vector<int> pos; long long ht = get_hash(t); long long hs = get_hash(s.substr(0, m)); if (hs == ht) return t.size(); for (int l = 1, r = m; r < n; l++, r++) { int del = ((s[l - 1] - 'a' + 1) * p) % M; int add = s[r] - 'a' + 1; hs = ((hs - del + M) * 31 + add) % M; if (hs == ht) return t.size(); } return INT_MAX; }
...
signed main () {
    ...
    while (t--) {
        ...
        cin >> s;
        int res = INT_MAX;
        rep (i,0,7) res = min(res, rabin_karp(s, vec[i]));
