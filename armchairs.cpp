...
signed main () {
	...
	rep (i,0,n) cin >> vec[i];
	vi start, end;
	rep (i,0,n) if (vec[i]) start.pb(i + 1); else end.pb(i + 1);
	int startsiz = start.size(), endsiz = end.size();
	if (startsiz == 0) {
		cout << 0;
		return 0;
	}
	int dp [startsiz][endsiz];
	rep (i,0,startsiz) rep (j,0,endsiz) dp[i][j] = inf;
	rep (i,0,startsiz) {
		rep (j,i,i + endsiz - startsiz + 1) {
			if (i == 0) dp[i][j] = abs(start[i] - end[j]);
			else dp[i][j] = abs(start[i] - end[j]) + dp[i - 1][j - 1]; 
			if (j > i) dp[i][j] = min(dp[i][j], dp[i][j - 1]);
		}
	}
	cout << dp[startsiz - 1][endsiz - 1];
